...
sigGenes=c("OSstatus","OS") #建一个向量，后面for循环输出用，因为后面还要用到surstat及surtime，所以先放在向量里
for(i in colnames(df[,3:ncol(df)])){ #从第3列开始循环，因为1列2列不是gene，是surstat和surtime
  tdcox <- coxph(Surv(OS, OSstatus) ~ df[,i], data = df)#开始逐一循环cox分析
...
outResult$id
# cox_result = coxph(formula = Surv(OS, OSstatus) ~ Age + Gender + TNM_stage + Vascular_invasion + Risk_Score, data = df, method="breslow")
# cox_result = coxph(formula = Surv(OS, OSstatus) ~ TNM_stage + Risk_Score, data = df, method="breslow")

cox_result = step(coxph(Surv(OS, OSstatus) ~ Tumor.Size + Tumor.Number + PVTT + TNM.Stage.AJCC. + CNLC.Stage + BCLC + AFP + CD161, data = df, method="breslow"), 
     direction = "both", 
     k = log(nrow(df)), 
     trace = 0)
