...
mcp = read.csv('../../data/MCPcounter/TCGA_LIHC_MCPCounter_score.csv', row.names = 1, check.names = F)
# mcp = log2(mcp + 1)
mcp <- mcp[row.names(mcp) != 'Fibroblasts', ]
mcp = data.frame(t(mcp), check.names = F)
mcp$patient = row.names(mcp)

data <- read.table("../../data/TCGA-LIHC/clean/TCGA-LIHC_tpm_protein_coding_clean_tumor.txt", header=T, sep="\t", check.names=F,row.names = 1)
...
write.csv(mcp, file="../../data/MCPcounter/TCGA-LIHC_MCPCounter_score_KLRB1.csv",row.names =T,quote=F)
...
ggsave(paste(result_dir, "TCGA_LIHC_MCPCounter_KLRB1_plot.pdf", sep=''),width = 30,height = 20,units = "cm")
